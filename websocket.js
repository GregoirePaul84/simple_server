...
async function getListenToken(symbol) {
    const res = await axios.post(
        'https://api.binance.com/sapi/v1/userListenToken',
        null,
        {
            params: { isIsolated: true, symbol },
            headers: { 'X-MBX-APIKEY': process.env.BINANCE_MARGIN_API_KEY }
        }
    );
    if (!res.data?.token) throw new Error(`Aucun listenToken reçu de Binance pour ${symbol}`);
    const minutesLeft = Math.round((res.data.expirationTime - Date.now()) / 60000);
    console.log(`ListenToken créé pour ${symbol}, expire dans ${minutesLeft} min`);
